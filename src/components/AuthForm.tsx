...
export function AuthForm({ onSuccess, defaultIsLogin = true }: AuthFormProps) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!isLogin) {
            if (parseInt(captchaInput) !== captcha.num1 + captcha.num2) {
                setErrors(prev => ({ ...prev, captcha: 'Incorrect math answer' }));
                // Reset captcha on failure to prevent brute force
        ...
        try {
            if (isLogin) {
                ...
                const { error } = await signUp(email, password); // Passed empty metadata object
                if (error) {
    ...
    return (
        <div className="w-full">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                ...
                {/* Math Captcha - Only for registration */}
                {!isLogin && (
                    <div className="space-y-2">
                        <div className="flex gap-4">
                            <div className="flex-1 bg-secondary/50 rounded-md flex items-center justify-center font-bold text-lg select-none">
